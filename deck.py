...
class Deck:
    def __init__(self, piles=[], card_images = {}, card_size = (80, 95)):
        self.cards = []
        ...
        # yellow
        self.empty_color = (100, 100, 250)

        self.piles = piles
        self.card_images = card_images
        self.card_size = card_size

        image_name = os.path.join('resources', 'card_back.png')
        self.card_back_image = pygame.image.load(image_name)
    ...
    def resize_card_images(self):
        for image_name, card_image in self.card_images.items():
            self.card_images[image_name] = pygame.transform.scale(card_image, self.card_size)

    def load_cards(self):
        for suit in self.suits:
            for rank in self.ranks:
                image_name = os.path.join('resources', 'cards', '{}_of_{}.png'.format(rank, suit))
                self.card_images[image_name] = pygame.image.load(image_name)
                self.cards.append(Card(image_name, self.card_size, rank, suit))
    ...
    def load_piles(self, display_size):
        ...
        start_x = 50
        start_y = self.card_size[1] + 50

        foundation_x_step = self.card_size[0] + pile_spacing
        foundation_start_x = display_width - (foundation_x_step * 4)

        tab1 = Pile([self.cards[0]], start_x, start_y, self.card_size)
        tab2 = Pile(self.cards[1:3], start_x + self.card_size[0] + pile_spacing, start_y, self.card_size)
        tab3 = Pile(self.cards[3:6], start_x + self.card_size[0]*2 + pile_spacing*2, start_y, self.card_size)
        tab4 = Pile(self.cards[6:10], start_x + self.card_size[0]*3 + pile_spacing*3, start_y, self.card_size)
        tab5 = Pile(self.cards[10:15], start_x + self.card_size[0]*4 + pile_spacing*4, start_y, self.card_size)
        tab6 = Pile(self.cards[15:21], start_x + self.card_size[0]*5 + pile_spacing*5, start_y, self.card_size)
        tab7 = Pile(self.cards[21:28], start_x + self.card_size[0]*6 + pile_spacing*6, start_y, self.card_size)
        ...
        self.piles = [tab1, tab2, tab3, tab4, tab5, tab6, tab7,
                      stock, waste,
    ...
    def display(self, game_display):
        for pile in self.piles:
            ...
            for card in pile.cards:
                ...
                if card.face_up:  
                    img = self.card_images[card.image_name]
                else:
                    img = self.card_back
